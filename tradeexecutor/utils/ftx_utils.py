...
async def ftx_fetch_borrow_rate_history(exchange, coin,start_time,end_time,params=None):
    request = {
        'coin': coin,
    ...
    response = await exchange.publicGetSpotMarginHistory(exchange.extend(request, params or dict()))
...
def ftx_collateralWeightInitial(future):
