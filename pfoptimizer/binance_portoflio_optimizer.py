...
# from riskpnl.binance_risk_pnl import *
# from utils.binance_utils import find_spot_ticker
from tradeexecutor.binance.api import BinanceAPI
from utils.config_loader import *
from histfeed.binance_history import get_history
...
async def refresh_universe(exchange, universe_filter):
    ...
    try:
        ...
        universe_end = (datetime(year=datetime.utcnow().year, month=datetime.utcnow().month,
                                 day=datetime.utcnow().day) - timedelta(days=1)).replace(tzinfo=timezone.utc)
        ...
        futures = await enricher(exchange,
                                 futures,
        ...
        # remove missing borrows and non USDT pairs
        selected_coin = {f for f in futures['spot_ticker'].unique()
                         if f.endswith(exchange.stablecoin)
                         and f"{f.split(exchange.stablecoin)[0]}_rate_borrow" in hy_history.columns}
        hy_history = hy_history[[col for col in hy_history.columns if col.split('_')[0] in selected_coin]]
        futures = futures.loc[futures['spot_ticker'].isin(selected_coin)]

        futures = market_capacity(futures, hy_history,
        ...
        sorted_screening_params = dict(
            sorted(screening_params.items(), key=lambda item: sum(item[1][v] for v in item[1].keys())))
        for tier in sorted_screening_params:
            population = futures.loc[(futures['spot_volume_avg'] > screening_params[tier]['spot_volume_threshold'])
                                     & (futures['future_volume_avg'] > screening_params[tier][
...
async def get_exec_request(run_type, exchange, **kwargs):
    ...
    dirname = os.path.join(os.sep, configLoader.get_config_folder_path(
        config_name=kwargs['config'] if 'config' in kwargs else None), "pfoptimizer")
    ...
    if run_type == 'spread':
        ...
        target_portfolio = pd.DataFrame(columns=['name', 'benchmark', 'target', 'exchange', 'subaccount', 'underlying'],
                                        data=[
                                            [cash_name, cash_price, float(kwargs['cash_size']), exchange, subaccount,
                                             coin],
                                            [future_name, future_price, -float(kwargs['cash_size']), exchange,
                                             subaccount, coin]])
...
async def perp_vs_cash(
    ...
    if backtest_start and backtest_end:
        ...
        trajectory = pd.DataFrame()
        pnl = pd.DataFrame()
        point_in_time = now_time.replace(hour=now_time.hour % 8, minute=0, second=0, microsecond=0)
    ...
    while point_in_time <= backtest_end:
        try:
            updated = update(filtered, point_in_time, hy_history, equity,
                             intLongCarry, intShortCarry, intUSDborrow, intBorrow, E_long, E_short, E_intUSDborrow,
                             E_intBorrow,
            ...
            previous_weights = optimized['optimalWeight'].drop(index=[exchange.stablecoin, 'total'])
            ...
            if backtest_start and backtest_end:
                ...
                pnl_list += [cash_flow[['name', 'end_time', 'bucket', 'amtUSD']].drop(
                    cash_flow[cash_flow['name'].isin([exchange.stablecoin, 'total'])].index)]
                ...
                # IR01
                cash_flow['amtUSD'] = (-cash_flow['previousWeight'] * (
                            (cash_flow['mark'] - cash_flow['spot']) - (prev_row['mark'] - prev_row['spot'])) / prev_row[
                                           'spot'])
    ...
    if backtest_start == backtest_end:
        pfoptimizer_path = os.path.join(os.sep, configLoader.get_config_folder_path(config_name=config_name),
                                        "pfoptimizer")
        ...
        pfoptimizer_res_filename = os.path.join(os.sep,
                                                pfoptimizer_path,
                                                'binance_optimal_cash_carry_' + datetime.utcnow().strftime(
                                                    "%Y%m%d_%H%M%S"))

        # stdout display
        display = optimized[['optimalWeight', 'ExpectedCarry', 'transactionCost']]
        totals = display.loc[[exchange.stablecoin, 'total']]
        display = display.drop(index=[exchange.stablecoin, 'total']).sort_values(by='optimalWeight',
                                                                                 key=lambda f: np.abs(f),
                                                                                 ascending=False).append(totals)
        logging.getLogger('pfoptimizer').info(display)

        # print to bus
        optimized = optimized.drop(
            index=[exchange.stablecoin, 'total']).fillna(0.0).sort_values(by='optimalWeight', key=lambda f: np.abs(f),
                                                                          ascending=False).head(
            config["NB_COINS"]["value"])
...
def to_json(exchange, all, config_name):
    ...
    subaccount = exchange.headers['binance-SUBACCOUNT']
...
async def strategy_wrapper(**kwargs):
    ...
    if isinstance(kwargs['equity_override'][0], (int, float)):
        equity_override = kwargs['equity_override'][0]
    elif kwargs['equity_override'][0] == "None":
        equity_override = None
    else:
...
def main(*args, **kwargs):
    '''
        examples:
            pfoptimizer sysperp binance subaccount=debug config=prod
            pfoptimizer basis binance type=future depth=100000
        args:
   ...
   '''
    ...
    #         df.to_csv(os.path.join(dirname, file),index=True,header=True,mode='w+')
    ...
    if run_type == 'sysperp':
        ...
        logger.critical(
            f'commands: sysperp [signal_horizon] [holding_period], backtest, depth [signal_horizon] [holding_period]')
    return res
