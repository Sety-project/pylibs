...
from sklearn.linear_model import LinearRegression,LassoLarsIC,RidgeCV,ElasticNetCV,LassoCV,LassoLarsCV, LogisticRegressionCV
        ...
        for (instrument, feature, _), data in original_data.items():
            for i in self.feature_map[feature]['ewma_windows']:
                ...
                self.data[(instrument, feature, 'live')].transform(
                    lambda x: x.ewm(times=x.index, halflife=timedelta(hours=i + 1)).mean()).dropna().rename((instrument, feature, f'ewma_{i}')) # x.shift(periods=i))#
                  ...
                  'run_parameters': {'verbose': False,
                                     ...
                                     'models': {'performance': [],#LinearRegression(),
                                                ...
                                                'sign': [LogisticRegressionCV(cv=TimeSeriesSplit(9),max_iter=1000)],
                                                        ...
                                                        #AdaBoostClassifier()],
                                                'big': [],#LogisticRegressionCV(cv=TimeSeriesSplit(9), max_iter=1000),
